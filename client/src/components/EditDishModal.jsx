...
function EditDishModal({ dish, onClose, onSuccess }) {
  const [dishName, setDishName] = useState(dish.dishName)
  const [freeFrom, setFreeFrom] = useState(
    allergens.filter((allergen) => dish.freeFrom.includes(allergen.toLowerCase()))
  )
  const [modifications, setModifications] = useState(
    dish.otherModifications
      ? [...dish.modifications, 'Other']
      : dish.modifications
  )
  const [otherModifications, setOtherModifications] = useState(dish.otherModifications || '')
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/dishes/${dish._id}`, {
        method: 'PUT',
        ...
        body: JSON.stringify({
          ...
          otherModifications: modifications.includes('Other') ? otherModifications : '',
        }),
      })
      ...
      <div className='modal' onClick={(e) => e.stopPropagation()}>
        ...
        <div className='modal-header'>
          <div>
            ...
            <p className='modal-subtitle'>{dish.restaurantName}</p>
          </div>
